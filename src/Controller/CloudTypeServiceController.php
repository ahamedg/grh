...
class CloudTypeServiceController extends AbstractController
{
    ...
    {
        $manager = $this->getDoctrine()->getManager();
        $cloudTypeService = $manager->getRepository(CloudTypeService::class)->find($id);
        $form = $this->createForm(CloudTypeServiceFormType::class, $cloudTypeService, array(
            'action' => $this->generateUrl('cloudTypeservice_edit', array('id' => $id)),
        ));
        ...
        return $this->render('cloud_type_service/index.html.twig', [
            'idTypeService' => $id,
            'cloudTypeServiceForm' => $form->createView()
        ]);
    }
